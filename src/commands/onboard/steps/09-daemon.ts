...
import { DEFAULT_PORT, isLaunchdAvailable, isSystemdAvailable, randomToken, waitForGateway } from "../helpers.js";
...
export const daemonStep: OnboardStep = async (ctx: OnboardContext) => {
  ...
  if (installService) {
    ...
    if (result?.ok) {
      ...
      await note(
        [`Error: ${result?.error}`, "", "You can start the daemon manually:", pc.cyan("  wopr daemon start")].join(
          "\n",
        ),

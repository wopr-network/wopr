// Embedding provider factory - adapted from OpenClaw (removed OpenClaw config deps)
// node-llama-cpp types (optional dependency)
type Llama = any;
...
async function importNodeLlamaCpp(): Promise<{
  getLlama: (opts: { logLevel: number }) => Promise<Llama>;
...
function formatLocalSetupError(err: unknown): string {
  ...
  return [
    ...
    missing
      ? "2) Reinstall WOPR with node-llama-cpp: npm i node-llama-cpp"
      : null,
    "3) If you use pnpm: pnpm approve-builds (select node-llama-cpp), then pnpm rebuild node-llama-cpp",
    'Or set memory.provider = "openai" (remote).',
  ]
    .filter(Boolean)
    .join("\n");
}

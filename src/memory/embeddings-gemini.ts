// Gemini embedding provider - adapted from OpenClaw (removed OpenClaw config deps)
...
const debugEmbeddings = process.env.WOPR_DEBUG_MEMORY_EMBEDDINGS === "1" ||
                        process.env.WOPR_DEBUG_MEMORY_EMBEDDINGS === "true";
...
const debugLog = (message: string, meta?: Record<string, unknown>) => {
  ...
  console.log(`[memory/embeddings] ${message}${suffix}`);
};
...
export async function resolveGeminiEmbeddingClient(
  ...
  const apiKey = remoteApiKey || process.env.GOOGLE_API_KEY?.trim() || process.env.GEMINI_API_KEY?.trim();
  if (!apiKey) {
    throw new Error("No API key found for provider google. Set GOOGLE_API_KEY or GEMINI_API_KEY environment variable.");
  }
